...
def dashboard():
    ...
    if period:
        ...
        registered_minors = registered_students - registered_adults
...
def new_culture_class():
    ...
    if request.method == 'POST':
        ...
        audience   = 'adult' if adult_only else 'children'
        try:
            cur.execute("""INSERT INTO class_group_record
...
def edit_culture_class(cid):
    ...
    if request.method == 'POST':
        ...
        audience    = 'adult' if adult_only2 else 'children'
        cur.execute("""UPDATE class_group_record SET pid=%s,name=%s,chinese_name=%s,
...
def edit_student(student_id):
    ...
    if request.method == 'POST':
        ...
        mfcs_aff        = f.get('mfcs_affiliation', 'N') or 'N'
        cur.execute("""UPDATE student SET
                ...
                is_adult=%s, mfcs_affiliation=%s WHERE id=%s""",
                (f.get('first_name',''), f.get('last_name',''),
                 ...
                 media_consent, is_adult_val, mfcs_aff, student_id))
        conn.commit(); conn.close()
